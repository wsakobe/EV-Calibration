...
namespace estimator
{
  namespace analytic_derivative
  {
    ...
    public:
      ...
      static inline MatN base_coefficients_ = computeBaseCoefficients<SplineOrder, double>();

      ///< Blending matrix. See \ref computeBlendingMatrix.
      static inline MatN blending_matrix_ = computeBlendingMatrix<SplineOrder, double, false>();
    };

    // typename RdSplineView::MatN RdSplineView::base_coefficients_ =
    //     estimator::computeBaseCoefficients<SplineOrder, double>();

    // typename RdSplineView::MatN RdSplineView::blending_matrix_ =
    //     estimator::computeBlendingMatrix<SplineOrder, double, false>();
